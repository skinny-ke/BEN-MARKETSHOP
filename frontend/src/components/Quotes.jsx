...
import { FaQuoteLeft, FaQuoteRight, FaHeart, FaStar, FaShoppingCart } from "react-icons/fa";
...
export default function Quotes({ 
  ...
  return (
    <motion.div
      className={`relative overflow-hidden rounded-lg p-6 ${variants[variant]} ${className}`}
      ...
      <div className="relative z-10">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {/* Quote Text */}
            <motion.blockquote 
              className={`text-lg md:text-xl font-medium leading-relaxed mb-4 ${textVariants[variant]}`}
              initial={{ scale: 0.9 }}
              animate={{ scale: 1 }}
            ...
            >
              ...
              <cite className={`text-sm font-medium ${variant === 'hero' ? 'text-green-200' : 'text-gray-600'}`}>
                — {currentQuote.author}
              </cite>
              <div className="w-8 h-px bg-current opacity-30"></div>
            </motion.div>
