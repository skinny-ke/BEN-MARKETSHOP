 ...
 * @desc    Alias for current authenticated user's profile
 * @access  Private
 */
router.get('/me', clerkAuth, async (req, res) => {
  try {
    const user = await User.findById(req.user?.id || req.auth?.userId).select('-password');
...
router.get('/:userId', clerkAuth, async (req, res) => {
  try {
    ...
    const currentUserId = req.user?.id || req.auth?.userId;
